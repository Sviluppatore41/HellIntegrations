...
public class BlockCustomAccelerators extends BlockCraftingUnit {
    ...
    public String getUnlocalizedName(final ItemStack is) {
        for (int n = 0; n < names.length; n++) {
            return "tile" + HellCore.MODID.toLowerCase() + names[n] + "_crafting_accelerator";
        }
    ...
    public void registerBlockIcons(IIconRegister ir) {
        for (int x = 0; x < names.length; x++) {
            REGISTERED_ICONS[2 * x] = ir.registerIcon(HellCore.MODID.toLowerCase() + ":ae2/crafting_accelerators/" + names[x] + "_crafting_accelerator");
            REGISTERED_ICONS[2 * x + 1] = ir.registerIcon(HellCore.MODID.toLowerCase() + ":ae2/crafting_accelerators/" + names[x] + "_crafting_accelerator" + "Fit");
        }
