package foxiwhitee.HellIntegrations.integration.avaritia.blocks;
...
import foxiwhitee.HellIntegrations.integration.avaritia.client.gui.GuiCustomNeutronCollector;
import foxiwhitee.HellIntegrations.integration.avaritia.container.ContainerCustomNeutronCollector;
import foxiwhitee.HellIntegrations.integration.avaritia.helpers.INeutronCollector;
import foxiwhitee.HellIntegrations.integration.avaritia.tile.collectors.*;
...
public class BlockCustomNeutronCollector extends AEBaseBlock implements INeutronCollector {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        this.top = iconRegister.registerIcon(HellCore.MODID.toLowerCase() + ":neutronsmachines/" + this.getUnlocalizedName().replace("tile.", "") + "_top");
        this.sides = iconRegister.registerIcon(HellCore.MODID.toLowerCase() + ":neutronsmachines/" + this.getUnlocalizedName().replace("tile.", "") + "_side");
        this.front = iconRegister.registerIcon(HellCore.MODID.toLowerCase() + ":neutronsmachines/" + this.getUnlocalizedName().replace("tile.", "") + "_front");
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
        ...
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile != null)
            player.openGui(HellCore.MODID.toLowerCase(), GuiHandlers.customNeutronCollector, world, x, y, z);
        return true;
    }
