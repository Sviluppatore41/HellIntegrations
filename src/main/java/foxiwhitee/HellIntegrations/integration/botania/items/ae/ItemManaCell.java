...
import foxiwhitee.HellIntegrations.integration.botania.helpers.IManaStorageCell;
import foxiwhitee.HellIntegrations.integration.botania.me.ManaCellInventory;
import foxiwhitee.HellIntegrations.integration.botania.me.ManaCellInventoryHandler;
...
public class ItemManaCell extends Item implements IManaStorageCell {
    ...
    public void registerIcons(IIconRegister getItemDamage) {
        for (int i = 0; i < prefixes.length; i++) {
            icons[i] = getItemDamage.registerIcon(HellCore.MODID.toLowerCase() + ":storageCells/" + name + prefixes[i]);
        }
    }
