package foxiwhitee.HellIntegrations.integration.botania.items.generating;

import foxiwhitee.HellIntegrations.HellCore;
import foxiwhitee.HellIntegrations.integration.botania.flowers.*;
import foxiwhitee.HellIntegrations.integration.botania.flowers.logic.generating.*;
...
public class ItemGenerationFlowerCores extends Item implements ICoreGeneratingFlower {
    ...
    public void registerIcons(IIconRegister register) {
        for (int i = 0; i < names.length; i++) {
            icons[i] = register.registerIcon(foxiwhitee.HellIntegrations.HellCore.MODID.toLowerCase() + ":botania/cores/generation/" + names[i]);
        }
    }
